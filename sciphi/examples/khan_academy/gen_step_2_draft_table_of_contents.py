...
from sciphi.examples.khan_academy.prompts import (
...
class DraftTableOfContentsRunner:
    ...
    def __init__(
        ...
        input_rel_dir: str = "output_step_1",
        output_rel_dir: str = "output_step_2",
        data_directory=None,
        log_level: str = "INFO",
    ):
        ...
        for yml_file_path in glob(f"{input_dir}/*.yaml"):
            try:
                ...
                with open(yml_file_path, "r") as yfile:
                    yml_load = yfile.read()
                    course_name = " ".join(yml_file_name.split("_")[:-1])
                ...
                yaml_completion = prase_yaml_completion(
                    {"completion": completion}
                )
                yml_load = yaml.safe_load(yaml_completion)
                save_yaml(yml_load, yml_output_file)
